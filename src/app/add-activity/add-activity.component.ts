...
export class AddActivityComponent implements OnInit, OnDestroy {
  ...
  private _GENERICIMAGE: IImage = {
    Url: "assets/images/480px-Image-x-generic-2.svg",
    altText: "Generic Image",
    height: 480,
    width: 480
  };
    ...
    this.$currentActivity = this.route.paramMap.pipe(
      switchMap((params: ParamMap) => {
        if (params.has("type")) {
          ...
          return this.as.getActivitybyKey(
  ...
  patchgeneralForm(act) {
    this.generalForm.patchValue({
      activetype: this.currentActivityType,
      general: this.genPropsfromActivity(act)
    });
  }
  ngOnDestroy() {
  ...
  saveAndclose({ value, valid }: { value: IAddActivity; valid: boolean }) {
    switch (value.activetype) {
      case ACTIVETYPE.Class:
        this.createAndsaveClass(value, valid);
        break;
      case ACTIVETYPE.DevProject:
        this.createAndsaveDev(value, valid);
        break;
      case ACTIVETYPE.NonProfit:
        this.createAndsaveNonProfit(value, valid);
        break;
      case ACTIVETYPE.Presentation:
        this.createAndsavePresent(value, valid);
        break;
      case ACTIVETYPE.Investment:
        this.createAndsaveInvest(value, valid);
        break;
      default:
        console.log("error");
        break;
  ...
  updateOrCreate(a: IActivity) {
    const ia = a as IActivity;
    if (!this.editmode) {
      this.as.createActivity(ia).then(
        av => this.updateEditfromCreatepromise(ia),
        e => {
      ...
      this.as
        .updateActivity(ia)
        .then(__ => this.updateEditfromCreatepromise(ia))
        .catch(e => {
  ...
  */
  uploadImageAndCreateOrSaveActivity(a: IActivity, v: IAddActivity): void {
    if (v.general.uploadfiles && v.general.uploadfiles.length > 0) {
      this.as
        .uploadImagefile(v.general.uploadfiles[0])
        .then(snap => {
  ...
  createAndsaveDev(value: IAddActivity, valid: boolean) {
    const orglink: ILink = {
      label: value.devproject.projectLabel,
      Url: value.devproject.projectUrl
    };
    const dev = new DevProjectActivity(value.general, orglink);
    dev.repository = {
      label: value.devproject.gitUrl,
      Url: value.devproject.gitUrl
    };
  ...
  patchDev(dev: DevProjectActivity) {
    this.generalForm.patchValue({
      devproject: {
        projectLabel: dev.organization.label,
        projectUrl: dev.organization.Url,
        gitUrl: dev.repository.Url
      }
    });
  }
  ...
  createAndsaveNonProfit(value: IAddActivity, valid: boolean) {
    const orglink: ILink = {
      label: value.nonprofit.orgLabel,
  ...
  patchNonProfit(np: NonProfitActivity) {
    this.generalForm.patchValue({
      nonprofit: {
        orgLabel: np.organization.label,
        orgUrl: np.organization.Url
      }
    });
  }
  createAndsavePresent(value: IAddActivity, valid: boolean) {
    const orglink: ILink = { label: null, Url: null };
    const prez = new PresentationActivity(value.general, orglink);
    prez.presentation = {
      label: value.present.presentUrl,
      Url: value.present.presentUrl
    };
    this.uploadImageAndCreateOrSaveActivity(prez, value);
  }
  patchPrez(prez: PresentationActivity) {
    this.generalForm.patchValue({
      presentation: { orgLabel: prez.presentation, orgUrl: prez.presentation }
    });
  }
  createAndsaveInvest(value: IAddActivity, valid: boolean) {
    const orglink: ILink = {
      label: value.investment.companyLabel,
  ...
  patchInvestment(act: IActivity) {
    if (act.activetype === ACTIVETYPE.Investment) {
      ...
      this.generalForm.patchValue({
        investment: {
          companyLabel: anginv.organization.label,
          companyUrl: anginv.organization.Url,
  ...
  createAndsaveClass(value: IAddActivity, valid: boolean) {
    const orglink: ILink = {
      label: value.class.schoolLabel,
      Url: value.class.schoolUrl
    };
    const cls = new ClassActivity(value.general, orglink);
    cls.syllabus = {
      label: value.class.syllabusUrl,
      Url: value.class.syllabusUrl
    };
    cls.department = {
  ...
  patchClass(cl: ClassActivity) {
    this.generalForm.patchValue({
      class: {
        syllabusUrl: cl.syllabus.Url,
        syllabusLabel: cl.syllabus.label,
        departmentLabel: cl.department.label,
        departmentUrl: cl.department.Url
      }
    });
  }
